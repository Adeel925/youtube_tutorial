...
def get_selected_channel(selected_elements,label_scale):
  ...
  if(len(elements_nodes_1) >1):
    ...
    for element in elements_nodes_1:
      nodes.append(element["data"]["id"])
      pairs = [(nodes[i], nodes[j]) for i in range(len(nodes)) for j in range(i+1, len(nodes))]
      elements_edges_2=[]
      for pair in pairs:
        colo_ede=channel_catagory[pair[0]][0]
        if(G.has_edge(pair[0], pair[1])):
          #print(edge1, edge2)
          print(pair)
          element = {'data': {'source': str(pair[0]), 'target': str(pair[1]),'color':color_map[colo_ede]}}
          elements_edges_2.append(element)
...
@app.callback(Output('cytoscape', 'elements'),
              [Input('catagory_search', 'value'), #reading input for specific content category
               Input('my-slider', 'value'),  #reading the inpit value for subsriber range
...
app.layout = html.Div(children=[
  ...
  #nav division
    ...
    html.Div(id='node-info',style={"textAlign": "center", 
        ...
        'overflow': 'scroll','background-color': 'lightsteelblue',}),
